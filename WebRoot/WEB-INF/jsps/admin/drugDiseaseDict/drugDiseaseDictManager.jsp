...
<SCRIPT language=javascript>
		...
		function addItem(){
			window.location.href="<%=request.getContextPath()%>	/admin/drugDiseaseDict_inputDiseaseDict.Q";
		}
		function delItem(){
			var selectCheckbox=$("input[type=checkbox][name=diseaseItemId]:checked");
			if(selectCheckbox.length<1){
				art.dialog({
				    content: '请至少选择一条疾病信息删除',
				    okVal:'确定',
				    ok: true
				});
			}else{
				...
				art.dialog({
				    content: '你确定要删除选中的疾病信息吗？',
				    okVal:'确定',
				    ok: function () {
				    	$.ajax({
							url:"<%=request.getContextPath()%>/admin/drugDiseaseDict_delDiseaseDict.Q",
		...
		function updateDisease(id){	
 			var url = "<%=request.getContextPath()%>/admin/drugDiseaseDict_inputDiseaseDict.Q";
 			if(id){
 				url+="?id="+id;
 			}
			...
			<div class="widget-box">
				<div class="widget-box">
					<div class="widget-content">
						<form action="<%=request.getContextPath()%>/admin/drugDiseaseDict_diseaseDictManager.Q"
							method="post" id="diseaseQueryForm">
							<div class="controls controls-row">
								<label class="span1" for="inputSuccess" style="margin-top:5px">项目名称:</label>
								<input name="dicDiseaseName" id="dicDiseaseName" type="text"
									class="span2" style="margin-left:5px;margin-right:35px"
									value="${dicDiseaseName}">
					...
					<div class="widget-title">
	           		...
	           		<s:if test='#request.pop != "1"'>
			            ...
			            <button class="label label-info btn btn-primary btn-mini" onclick="delItem();">删除</button>
						 ...
						 <div class="widget-content nopadding" id="diseaseTableId">
	          					  <%@include file="drugDiseaseDictList.jsp"%>
	     				 </div>
					</div>
				</div>
			</div>
