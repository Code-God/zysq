<%@ page language="java" pageEncoding="UTF-8"%>
<%@ taglib prefix="s" uri="/struts-tags" %>
<%@ taglib prefix="c" uri="/WEB-INF/c.tld"%>
<%@ taglib prefix="tags" tagdir="/WEB-INF/tags" %>					
						<table class="table table-bordered table-striped">
							<thead>
								<tr>
									<th>疾病名称</th>
									<th>疾病分类</th>
									<th>疾病概要</th>
									<th>是否推荐</th>
									<th>时间</th>
									<th>操作</th>
								</tr>
							</thead>
							<tbody id="dataList">
								<s:if test="#request.diseaseList.size>0">
									<s:iterator value="#request.diseaseList" var="admin">
										<tr>												
											<td>
											<input type="checkbox" name="diseaseItemId" value="<s:property value="id"/>">
											<s:property value="diseaseName" />
											</td>
											<td><s:property value="classificationName" /></td>
											<td><s:property value="diseaseProfile" /></td>
											<td>
												<s:if test="#admin.isshow==1">
													是
												</s:if>
												<s:else >
													否
												</s:else>
											</td>
											<td><s:date name="updateTime" format="yyyy-MM-dd hh:mm:ss" /></td>
											<td><button	class="label label-info btn btn-primary btn-mini"
													onclick="updateDisease('<s:property value="id"/>');">修改</button></td>
										</tr>
									</s:iterator>
								</s:if>
								<s:else>
									<tr>
										<td colspan="11">
											<div class="alert alert-block">
												<h4 align="center" class="alert-heading">暂时没有符合条件的记录！</h4>
											</div>
										</td>
									</tr>
								</s:else>
							</tbody>
						</table>
						<tags:ajaxPagination scripts="ajaxGetDiseaseList" divId="diseaseTableId" queryFormId="diseaseQueryForm" page="${page }" ></tags:ajaxPagination>
						</tbody>
						</table>