...
<SCRIPT language=javascript>
		...
		function addItem(){
			window.location.href="<%=request.getContextPath()%>	/admin/durgProjectConter_inputProjectConter.Q";
		}
		function delItem(){
			var selectCheckbox=$("input[type=checkbox][name=projectId]:checked");
			if(selectCheckbox.length<1){
				...
				art.dialog({
				    ...
				    ok: function () {
				    	$.ajax({
							url:"<%=request.getContextPath()%>/admin/durgProjectConter_delProjectConterItem.Q",
		...
		function updateProjectConter(id){			
 			var url = "<%=request.getContextPath()%>/admin/durgProjectConter_inputProjectConter.Q";
 			if(id){
 				url+="?id="+id;
 			}
			...
			<div class="widget-box">
				<div class="widget-box">
					<div class="widget-content">
						<form action="<%=request.getContextPath()%>/admin/durgProjectConter_manager.Q"
							method="post" id="projectQueryForm">
							<div class="controls controls-row">
								<label class="span1" for="inputSuccess" style="margin-top:5px">机构名称:</label>
					...
					<div class="widget-title">
	           		...
	           		<s:if test='#request.pop != "1"'>
			            ...
			            <button class="label label-info btn btn-primary btn-mini" onclick="delItem();">删除</button>
						 ...
						 <div class="widget-content nopadding" id="projectTableId">
	          					  <%@include file="drugProjectConterList.jsp"%>
	     				 </div>
					</div>
				</div>
			</div>
