...
<SCRIPT language=javascript>
		...
		function addItem(){
			window.location.href="<%=request.getContextPath()%>	/admin/drugs_inputDrugs.Q";
		}
		function delItem(){
			var selectCheckbox=$("input[type=checkbox][name=drugsId]:checked");
			if(selectCheckbox.length<1){
				art.dialog({
				    content: '请至少选择一条药物信息删除',
				    okVal:'确定',
				    ok: true
				});
			}else{
				...
				art.dialog({
				    content: '你确定要删除选中的药物信息吗？',
				    okVal:'确定',
				    ok: function () {
				    	$.ajax({
							url:"<%=request.getContextPath()%>/admin/drugs_delDrug.Q",
		...
		function updateDrugs(id){			
 			var url = "<%=request.getContextPath()%>/admin/drugs_inputDrugs.Q";
 			if(id){
 				url+="?id="+id;
 			}
			...
			<div class="widget-box">
				<div class="widget-box">
					<div class="widget-content">
						<form action="<%=request.getContextPath()%>/admin/drugs_drugsManager.Q"
							method="post" id="diseaseQueryForm">
							<div class="controls controls-row">
								<label class="span1" for="inputSuccess" style="margin-top:5px">药物名称:</label>
					...
					<div class="widget-title">
	           		...
	           		<s:if test='#request.pop != "1"'>
			            ...
			            <button class="label label-info btn btn-primary btn-mini" onclick="delItem();">删除</button>
						 ...
						 <div class="widget-content nopadding" id="drugsTableId">
	          					  <%@include file="drugsList.jsp"%>
	     				 </div>
					</div>
				</div>
			</div>
