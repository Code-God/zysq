...
<HTML xmlns="http://www.w3.org/1999/xhtml">
	...
	function initList(page){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/admin.do?m=getKeyWords",
			...
			success : function(data) {
				if (data != "fail" && data != 'null') {
					...
					var list = page.datas;
					//表头
					var resumeList = "<tr style='thead'><TD width=50px>序号</TD><TD width=50px>规则名</TD><TD>关键字</TD><TD width=40px>回复类型</TD><TD>回复内容</TD><TD width=100px><INPUT onclick=selectallbox() type=checkbox id=checkAll name=checkAll>操作</TD></TR>";
	...
	function edit(id){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/admin.do?m=getRule",
			data: "kid="+ id,
			dataType : "json",
			success : function(data) {
				var r = eval(data);
			...
			<TBODY>
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
							...
							<TR>
								<TD height=2 style="color: red">
									模块说明：<br>
										...
										此模块实现了和微信官方一样的功能，支持关键字回复，回复方式支持以下三种：
									<hr>
									1）文本推送回复<br>
									2）单图文推送回复<br>
									3）多图文推送回复<br>
						...
						<html:form action="/admin?m=saveOrUpdateKeyWords" onsubmit="return validatePwd();" method="POST" styleId="form">
							<table width="780" border="0" align="center" cellSpacing=0 cellPadding=5 class="resTable">
								...
								<tr>
									<td colspan="4">
										<font color='#006400'>▋</font>关键字（请用"|"分割不同的关键字，<font color=red>禁止换行</font>，如：“hi|你好|在吗？|请问”）：
										<textarea rows="5" cols="120" name="keywords" id="keywords" style="overflow: auto"></textarea>
										<span class='org'>*</span>&nbsp;
									</td>
								</tr>
								<tr>
								...
								<tr>
									<td colspan="4" align="center">
										...
										<input type="submit" id="submitId" value="新增" class="hrbt">
										<input type="button" id="clear" value="清空" class="hrbt" onclick="clearForm()">
										&nbsp;<br/>
										<c:if test="${info != null}">
