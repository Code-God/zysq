...
<script type="text/javascript">
	...
	function initList(page){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/admin/admin_getImageKeyWords.Q",
			...
			success : function(data) {
				if (data != "fail" && data != 'null') {
					...
					var list = page.datas;
					//表头
					var resumeList = "<tr style='thead'><TD width=50px>序号</TD><TD width=60px>回复类型</TD><TD>回复内容</TD><TD width=100px><INPUT onclick=selectallbox() type=checkbox id=checkAll name=checkAll>操作</TD></TR>";
	...
	function edit(id){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/admin/admin_getImageRule.Q",
			data: "kid="+ id,
			dataType : "json",
			success : function(data) {
				var r = eval(data);
							...
							<TR>
								<TD height=2 style="color: red">
									模块说明：<br>
										...
										此模块实现了和微信官方一样的功能，支持用户输入图片消息回复，回复方式支持以下一种：<br>
									1）文本推送回复<br>
									<hr>
								</TD>
							</TR>
						</TABLE>
						<form action="<%=request.getContextPath() %>/admin/admin_saveOrUpdateImageKeyWords.Q"   method="POST" id="form">
