...
//$("#uploadFile").uploadPreview({width:50, height:50, imgDiv: "#image-id"});

function fileChange(obj) {alert(obj);
	$(obj).uploadPreview({width:50, height:50, imgDiv: "#image-id"});
}
(function($) {  
    jQuery.fn.extend({  
        uploadPreview: function(opts) {  
            opts = jQuery.extend({  
                ...
                imgDiv: "#imgDiv",  
                maxSize:300,  
                imgType: ["gif", "jpeg", "jpg", "bmp", "png", "GIF", "JPEG", "JPG", "BMP", "PNG"],  
        	...
        	var version = parseInt((browser+"").replace(/[^0-9.]/ig,"")); 
            ...
            autoScaling = function() {  
                ...
                if (img_width > 0 && img_height > 0) {  
                    ...
                    var img = $("#imagesLi").html();
                    var len = $("#figure-id").find("img").length;
                    
                    // 隐藏原file控件，新增一个新的file控件
                    $("#uploadFile").parent().hide();
                    $("#uploadFile").attr("id", "uf"+len);
                    // 添加缩略图，并新增加一个文件选择组件
                    var h = [];
                    h.push("<figure class=\"record\"><i class=\"iconfont icon-guanbi2\" onclick=\"removeFile(this);\"></i>");
                    h.push(img);
                    h.push("</figure>");
                    h.push("<span class=\"add_record iconfont icon-tianjia\"><input id=\"uploadFile\" name=\"myFile\" onchange=\"fileChange(this);\" type=\"file\"></span>");
                    $("#figure-id").after(h.join(""));
                    // 为新文件组件添加事件
                    //$("#uploadFile").uploadPreview({width:50, height:50, imgDiv: "#image-id"});
                }
            },  
