 ...
 */
// add by jove wjdeng 日期控件失去焦点后的校验调整
function checkdate(dp) {
	try {
		if (this.value.length != 0) {
			var tip = document.getElementById(this.id + 'Tip');
			tip.className = 'onCorrect';
			tip.innerHTML = JS_I18N.wdatePicker_is_correct;
		} else {
			var tip = document.getElementById(this.id + 'Tip');
			tip.className = 'onError';
			tip.innerHTML = JS_I18N.wdatePicker_is_null;
		}
	} catch (e) {
	}
}
