...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		var settings = 
		{
			...
			tidymode:false,
			errorfocus:true,
			wideword:false//change by jonim to deal zh_CN length
		};
        ...
        switch(sType)
		{
			...
			case "file":
				...
				if (initConfig.wideword)
				{
					...
					len = val.replace(/(^\s*)|(\s*$)/g,"").length;//add by jonim 去除空格
				}
		        break;
			case "checkbox":
			case "radio": 
...
$.fn.formValidator = function(cs) 
{
	var setting = 
	{
		...
		submitonce : false,
		automodify : false,
		onshow :JS_I18N.please_input,//"请输入内容",
		onfocus: JS_I18N.please_input,//"请输入内容",
		oncorrect: JS_I18N.input_ok,//"输入正确",
		onempty: JS_I18N.wdatePicker_is_correct,//"输入内容为空",
		defaultvalue : null,
		bind : true,
...
$.fn.inputValidator = function(controlOptions)
{
	var settings = 
	{
		...
		max : 99999999999999,
		type : "size",
		onerror:JS_I18N.input_error,//"输入错误",
...
$.fn.regexValidator = function(controlOptions)
{
	var settings = 
	{
		...
		param : "i",
		datatype : "string",
		onerror:JS_I18N.format_illegal,//"输入的格式不正确",
